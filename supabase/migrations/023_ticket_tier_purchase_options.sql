...
BEGIN
  ...
  FOR tier_record IN
    SELECT tiers.id, tiers.price, tiers.quantity, tiers.sold, tiers.group_size, tiers.expires_at
  ...
  LOOP
    ...
    IF tier_record.expires_at IS NOT NULL AND tier_record.expires_at < CURRENT_DATE THEN
      RAISE EXCEPTION 'Ticket tier % has expired', tier_record.id;
    END IF;

    issued_quantity := requested_quantity * GREATEST(1, tier_record.group_size);
    IF tier_record.sold + issued_quantity > tier_record.quantity THEN
    ...
    VALUES (created_order_id, tier_record.id, issued_quantity, tier_record.price, requested_quantity * tier_record.price);
    ...
    FROM generate_series(1, issued_quantity);

    UPDATE ticket_tiers
    SET sold = sold + issued_quantity
