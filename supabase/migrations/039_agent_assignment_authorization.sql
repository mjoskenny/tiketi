-- An active assignment is the authoritative agent grant for sales.
-- Older invitation data can leave organizer_members pending even after the
-- assignment has been activated, so do not require a second stale membership state.
...
BEGIN
  ...
  END IF;
  IF p_payment_mode NOT IN ('mobile_money', 'card', 'bank') THEN
